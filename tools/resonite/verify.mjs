...
let bad = 0, mirrored = 0, unescaped = 0, tofu = 0;
...
// The bundled typefaces are text fonts and carry no emoji, and the engine has no fallback
// chain, so an emoji inside a TextRenderer draws as a NO GLYPH box. A run containing one has
// to be exported as a picture of itself instead.
...
(function strings(s, path) {
  for (const c of d(s.Components) ?? [])
    for (const [k, v] of Object.entries(c.Data)) {
      ...
      if (k === 'Text' && /TextRenderer/.test(T[c.Type]) && typeof val === 'string' && EMOJI.test(val)) {
        console.log(`${path}/${d(s.Name)} Text = ${JSON.stringify(val)}` +
                    '  *** no bundled font has these glyphs — they will draw as NO GLYPH boxes ***');
        tofu++;
      }
    }
...
if (bad || mirrored || unescaped || tofu) {
  if (unescaped) console.error(`${unescaped} string field(s) start with a bare '@' and will not load`);
  if (tofu) console.error(`${tofu} text run(s) carry emoji no bundled font can draw`);
...
console.log(`\nok — ${quadRot.size} quads, no collider reaches the far face, nothing mirrored, ` +
            'no string field misreads as a URL, no emoji in text');
